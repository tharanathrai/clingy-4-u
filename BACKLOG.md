...
5. PWA install shipped (service worker via `vite-plugin-pwa`, installable on Android + iOS, offline shell, reload prompt). Deferred follow-ups:
   - ~~Push notifications~~ shipped as Web Push/VAPID in spec `022` (code merged; owner ops pending — see spec Dependencies). Icon badge shipped same day (Badging API). Still deferred: per-type push preferences (needs a server-side prefs table; the email toggles in Settings are localStorage-only no-ops).
