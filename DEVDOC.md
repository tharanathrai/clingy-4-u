...
- What works: Notification list with unread count; real-time INSERT direct cache patch via `setQueryData`; real-time UPDATE patch in-place; all via `subscribePostgresChannel`; mark-as-read/mark-all/dismiss via optimistic `useMutation` with rollback; skeleton loading (4 rows); empty state ("All caught up."); error state with retry; routing to correct destination per type; stale gum-piece taps on terminal statuses (`confirmed`, `turned_down`, `expired`) auto-dismiss with toast for `/piece/:id` route types including `invite_accepted`, `plan_turned_down` (C-03) — `plan_expired` is excluded: copy reads "Your plan with {name} expired" on two-person plans (other member's name denormalized into `actor_name` by `run-expiry`) and "Your group plan expired" on group plans (`actor_name` null) and tap opens `ExpiredPlanSheet` (title, other members, expired-ago) with **Plan it again** → `/piece/new` prefilled via `recipientIds` + `initialTitle` state (back → `/notifications`) and **See graveyard** → `/home/graveyard` with `returnTo: '/notifications'`; every row has an X (`Dismiss notification`) wired to `dismissNotification`; dismiss requires the `notifications_delete_own` RLS policy (migration 20260921000000); `post_reaction` intentionally excluded (PRD section 14); `plan_expired` included in enrichNotifications gumPieceIds filter.
...
- **Web Push (spec `022`, $0 via VAPID):** `notifications` INSERT → `notifications_push_after_insert` trigger (skips `post_reaction`; no-op until Vault `send_push_secret` exists) → pg_net → `send-push` edge fn (`jsr:@negrel/webpush`, 202 + `EdgeRuntime.waitUntil`, deletes `push_subscriptions` rows on 404/410) → `src/sw.ts` `push` → `showNotification` → `notificationclick` opens `/notifications`. Client: `src/lib/push.ts` (`subscribeToPush` / `unsubscribeFromPush`, truth = SW subscription), `src/lib/pushSupport.ts` (pure support matrix), Settings "Push notifications" toggle (hidden when unsupported; iOS Safari tab shows add-to-Home-Screen hint). Copy shared via `supabase/functions/_shared/notificationCopy.ts` + `pushPayload.ts`. Icon badge: `send-push` puts the recipient's unread count in the payload → SW `navigator.setAppBadge(unread)`; `useNotifications` re-syncs via `src/lib/appBadge.ts` whenever `unreadCount` changes (reading clears it). Live 2026-09-17. `npm run build` runs `scripts/verify-sw.mjs`, which evaluates `dist/sw.js` in a stub scope — a throwing SW script otherwise fails registration silently (no SW, no push, no offline shell). Push failures are silent — check `select status_code, left(content,200), created from net._http_response order by created desc limit 5`. Ops checklist in the spec's Dependencies.
...
All 16 edge functions manually validate the JWT by calling `supabase.auth.getUser(token)` (or service-role bearer for cron/email/analytics; `run-expiry` also accepts the dedicated `RUN_EXPIRY_SECRET` and `send-push` the dedicated `SEND_PUSH_SECRET` — both read from Vault by Postgres, see `supabase/scripts/schedule-run-expiry.sql` and `supabase/scripts/setup-send-push.sql`). Intentional for error message flexibility. **Config drift:** `config.toml` registers only 14 — `edit-gum-piece` and `track-events` still need `[functions.*]` entries.

Edge secrets in use: `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `RESEND_API_KEY`, `RESEND_FROM_EMAIL`, `ANALYTICS_SALT`, `RUN_EXPIRY_SECRET`, `VAPID_KEYS_JSON` (JWK pair from `@negrel/webpush` `generate-vapid-keys.ts`), `VAPID_SUBJECT` (`mailto:`), `SEND_PUSH_SECRET`. Client env: `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`, `VITE_VAPID_PUBLIC_KEY` (missing → push toggle hidden, nothing throws).
...
15. **Web Push delivery** — After the spec `022` ops steps: Android Chrome (tab + installed) and iOS 16.4+ installed PWA. Settings → Push notifications → OS prompt → accept → `push_subscriptions` row. From a second account send an invite: notification arrives with the app closed; tap opens `/notifications` and routes to the piece. Toggle off → row deleted. Revoke permission in browser settings → next send 410 → row auto-deleted. iOS Safari tab shows the add-to-Home-Screen hint and no prompt. "New version available" banner + Reload still work after the `injectManifest` switch. **Status: pending-live**
