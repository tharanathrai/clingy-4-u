...
This file started as the Week 1 build plan. Current codebase status:

- ✅ Routing/auth foundation is in place in `src/App.tsx`, `src/main.tsx`, and `src/components/layout/AuthGuard.tsx`.
- ✅ Google OAuth flow, callback, and onboarding are implemented in `src/hooks/useAuth.ts`, `src/pages/Landing.tsx`, `src/pages/AuthCallback.tsx`, and `src/pages/Welcome.tsx`.
- ✅ QR add/scan and connection-request flows are implemented in `src/pages/Add.tsx`, `src/pages/AddScan.tsx`, `src/pages/ConnectionRequests.tsx`, `supabase/functions/generate-qr-token/index.ts`, and `supabase/functions/validate-qr-token/index.ts`.
- ✅ Core authenticated shell is implemented in `src/components/layout/Layout.tsx` and `src/components/layout/BottomTabBar.tsx`.
- ⚠️ Delta from original plan: Apple sign-in is not currently wired in `useAuth`/`Landing` (Google-only in UI).
- ⚠️ `src/pages/Connect.tsx` and `src/pages/Settings.tsx` remain `PageStub` placeholders.
