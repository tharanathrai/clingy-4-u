...
This file started as the Week 2 build plan. Current codebase status:

- ✅ Gum lifecycle edge functions are implemented in `supabase/functions/categorize-gum/index.ts`, `supabase/functions/create-gum-piece/index.ts`, and `supabase/functions/respond-gum-piece/index.ts`.
- ✅ Slot limits, category assignment, notification creation, and gum shape persistence are live in the create/respond flow.
- ✅ Pocket UI and piece flows are implemented in `src/pages/Home.tsx`, `src/pages/PieceNew.tsx`, and `src/pages/PieceDetail.tsx`.
- ✅ Notifications hook/page/components are implemented in `src/hooks/useNotifications.ts`, `src/pages/Notifications.tsx`, and `src/components/notifications/NotificationItem.tsx`.
- ✅ Core gum UI components are implemented in `src/components/gum/GumPieceCard.tsx` and `src/components/gum/CategoryChip.tsx`.
- ℹ️ RLS SQL captured in `supabase/week2-rls-policies.sql`; graveyard is no longer a stub and is implemented in Week 3 files.
